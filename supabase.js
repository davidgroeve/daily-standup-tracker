...
(function initSupabase() {
    ...
    window.supabaseClient = window.supabase.createClient(
        'https://ksveqaogqrtuihtyiqbj.supabase.co',
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtzdmVxYW9ncXJ0dWlodHlpcWJqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjQ4NDg3MzgsImV4cCI6MjA4MDQyNDczOH0.hk6rG0xYolG-og71sd1JBW5HUMdkLZwC2ei2QE9uXZg'
    );

    // Database Service - uses window.supabaseClient throughout
    window.db = {
        // Team Members
        ...
        async createTeamMember(member, userEmail) {
        ...
        async updateTeamMember(id, updates, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { data, error } = await window.supabaseClient
                .from('team_members')
                .update(updates)
        ...
        async deleteTeamMember(id, userEmail) {
            userEmail = userEmail || 'Unknown';
        ...
        async getUpdates() {
            ...
            (data || []).forEach(function (update) {
                var key = update.member_id + '_' + update.date;
                updates[key] = {
        ...
        async saveUpdate(memberId, date, content, items, timestamp, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { error } = await window.supabaseClient
                .from('updates')
                .upsert({
        ...
        async deleteUpdate(memberId, date, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { error } = await window.supabaseClient
                .from('updates')
                .delete()
        ...
        async createGoal(goal, weekStart, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { data, error } = await window.supabaseClient
                .from('goals')
                .insert([{
        ...
        async updateGoal(id, updates, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { data, error } = await window.supabaseClient
                .from('goals')
                .update(updates)
        ...
        async createLeave(leave, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { data, error } = await window.supabaseClient
                .from('leaves')
                .insert([{
        ...
        async updateLeave(id, updates, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { data, error } = await window.supabaseClient
                .from('leaves')
                .update(updates)
        ...
        async deleteLeave(id, userEmail) {
            userEmail = userEmail || 'Unknown';
            const { error } = await window.supabaseClient
                .from('leaves')
                .delete()
